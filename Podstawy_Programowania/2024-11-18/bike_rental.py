...
def cancel_rental(customer_name):
    rentals = load_json(RENTALS_FILE)
    rental = next((r for r in rentals if r["customer_name"] == customer_name), None)
    
    if rental:
        rentals.remove(rental)
        save_json(RENTALS_FILE, rentals)
        print(f"Rental for {customer_name} has been cancelled.")
    else:
